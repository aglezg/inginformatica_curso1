...
void 
GRAFO::MostrarCamino(unsigned s, unsigned i, vector<unsigned> pred) //función para mostrar los caminos del algoritmo de Dijkstra
{
  if(i!=s)
  {
...
void GRAFO::Kruskal()//__________
{
 ...
 if(Aristas.size() != 0){ //debemos asegurarnos que el grafo contiene aristas
    ...
    cout << endl << "Este grafo no posee aristas." << endl;
...
void GRAFO::Dijkstra()//__________
{
...
PermanentementeEtiquetado.resize(n,false); //inicialmente no hay ningún nodo permanentemente etiquetado
d.resize(n,maxint);                        //inicialmente todas las etiquetas distancia son infinito
pred.resize(n,UERROR);                     //inicialmente el pred es null

do{
cout<< "Caminos mínimos: Dijkstra" << endl; //solicitamos al usuario el nodo de origen
cout << "Nodo de partida? [1-" << n << "]: ";
cin >> (unsigned &) s;
if(s<1 || s>n)
 system("clear");
}while(s<1 || s>n); //me aseguro de que el nodo exista

d[--s]=0; pred[s]=s;

candidato=s; //candidato = 0 = "nodo 1"

/*desarrollo*/

do{

 min=maxint;//la distancia mínima la establecemos a infinito (maxint)

 for(int i=0; i<PermanentementeEtiquetado.size(); i++) //recorro todo el vector "PermanentementeEtiquetado" y por consiguiente todos los nodos
  if (PermanentementeEtiquetado[i]==false)             //buscando el de menor etiqueta distancia no etiquetado
   if (d[i]<min){
   candidato=i;
   min=d[i];
   }

   PermanentementeEtiquetado[candidato]=true; //establezco la etiqueta del nodo candidato (con la menor etiqueta distancia) a "true"
                                              //si todos los nodos estaban etiquetados el "candidato" que estaba ya a "true" se establecerá de nuevo en "true"
                                              //por lo que no ocurrirá nada 

  if(LS[candidato].size()!=0) 
   for(int j=0; j<LS[candidato].size(); j++) //recorro la lista de sucesores del nodo "candidato" si es que tiene sucesores
    if((d[LS[candidato][j].j] > (d[candidato]+LS[candidato][j].c)) && (PermanentementeEtiquetado[LS[candidato][j].j]==false)){ //si (d[j] > d[i]+c[i,j] y j no etiquetado)
     d[LS[candidato][j].j]=(d[candidato]+LS[candidato][j].c); //actualizo su vector "d"(distancia)
     pred[LS[candidato][j].j]=candidato;                      // y su vector "pred"(predecesor)
     }
...
for(int i=0;i<pred.size();i++){ //recorremos el vector de predecesores para mostrar los caminos

if(d[i]!=maxint){ //si se actualizó el vector "distancia" existe un camino para ese nodo
 if(i!=s){                                          
cout << "El camino del nodo "<< s+1 << " al nodo "<< i+1 <<" es: ";
MostrarCamino(s,i,pred);
cout << i+1 << " de longitud " << d[i] << endl;
}

}else //si no se actualizó el vector de distancia es que no existe camino
